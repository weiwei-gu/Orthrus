...
- **sim2sim 漂移的两面（DR 终局判决）**：物理参数定值训练越过 ~12M 步后 MuJoCo 迁移性单调退化；
...
- DR 终局判决：行走段 sim2sim 漂移已关闭，但推力训练策略的转移死亡未解（接触求解器层缺口）——
  下一轮方向：solref/solimp 进 DR、部署侧接触适配、或推力训练加 MuJoCo 快照准入门槛
